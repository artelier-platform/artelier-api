...
public class OrderController {
    ...
    @ApiResponse(
            ...
            content = @Content(
                    mediaType = MediaType.APPLICATION_JSON_VALUE,
                    schema = @Schema(implementation = SwaggerResponses.OrderResponseBody.class),
    ...
    @GetMapping("/my")
    @Operation(summary = "Get my orders", description = """
    ...
    @ApiResponse(
            ...
            content = @Content(
                    mediaType = MediaType.APPLICATION_JSON_VALUE,
                    schema = @Schema(implementation = SwaggerResponses.OrderListResponseBody.class),
                    examples = @ExampleObject(name = "with-orders", value = """
                            {
                              "success": true,
    ...
    @ApiResponse(
            ...
            content = @Content(
                    mediaType = MediaType.APPLICATION_JSON_VALUE,
                    schema = @Schema(implementation = SwaggerResponses.OrderPageResponseBody.class),
                    examples = @ExampleObject(name = "paginated-orders", value = """
                            {
                              "success": true,
                              "message": "Orders fetched",
                              "data": {
    ...
    public ResponseEntity<AppResponse<Page<OrderResponse>>> getAllOrders(
            @Parameter(description = "Filter by order status. Optional — omit to return all statuses.",
    ...
    @ApiResponse(
            ...
            content = @Content(
                    mediaType = MediaType.APPLICATION_JSON_VALUE,
                    schema = @Schema(implementation = SwaggerResponses.OrderResponseBody.class),
                    examples = @ExampleObject(name = "status-updated", value = """
                            {
                              "success": true,
                              "message": "Order status updated",
                              "data": { "id": "770e8400-e29b-41d4-a716-446655440002", "status": "SHIPPED" }
                            }
                            """)
            )
    )
    ...
    @ApiResponse(responseCode = "403", description = "Buyer attempted to modify another user's order",
            content = @Content(mediaType = MediaType.APPLICATION_JSON_VALUE, schema = @Schema(implementation = AppResponse.class),
                    examples = @ExampleObject(value = """
                            {
                             "success": false,
                             "message": "Access denied to this order"\s
                             }
                            \s""")))
    ...
    public ResponseEntity<AppResponse<OrderResponse>> updateStatus(
            @Parameter(description = "Order UUID", required = true, example = "770e8400-e29b-41d4-a716-446655440002")
            @PathVariable UUID id,
            @Parameter(description = "Target status", required = true, schema = @Schema(implementation = OrderStatus.class))
    ...
    @Operation(summary = "Get order by ID", description = """
            ...
            - `BUYER` can only retrieve their own orders.
            """)
    @ApiResponse(
            responseCode = "200",
            ...
            content = @Content(
                    mediaType = MediaType.APPLICATION_JSON_VALUE,
                    schema = @Schema(implementation = SwaggerResponses.OrderResponseBody.class),
                    examples = @ExampleObject(name = "order-detail", value = """
                            {
                              "success": true,
    ...
    @ApiResponse(responseCode = "403", description = "Buyer attempted to access another user's order",
            content = @Content(mediaType = MediaType.APPLICATION_JSON_VALUE, schema = @Schema(implementation = AppResponse.class),
                    examples = @ExampleObject(value = """
                            {
                             "success": false,
